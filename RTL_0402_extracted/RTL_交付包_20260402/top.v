...
module top (
    ...
    reg erase_pending_prev;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) erase_pending_prev <= 1'b0;
        else        erase_pending_prev <= erase_key_pressed ? 1'b1 : (flash_done ? 1'b0 : erase_pending_prev);
    end
    wire erase_done_pulse = erase_pending_prev & flash_done;
    ...
    // 动态补偿就绪判定：GPS稳定后先等待3个16秒窗口，再用后3个16秒窗口求平均并锁定参数
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            dynamic_comp_ready <= 1'b0;
            comp_settle_cnt <= 2'd0;
            comp_init_sum <= 34'sd0;
            comp_init_cnt <= 2'd0;
            ...
            if (!comp_param_locked) begin
                // 先稳定等待3个窗口，不参与训练
                if (comp_settle_cnt < 2'd3) begin
                    comp_settle_cnt <= comp_settle_cnt + 2'd1;
                    comp_init_sum <= 34'sd0;
                    comp_init_cnt <= 2'd0;
                end else begin
                    // 再采3个窗口求平均并锁定
                    comp_init_sum <= comp_init_sum + $signed(diff_total[31:0]);
                    if (comp_init_cnt == 2'd2) begin
                        comp_param_fixed <= $signed((comp_init_sum + $signed(diff_total[31:0])) / 3);
                        comp_param_locked <= 1'b1;
                        dynamic_comp_ready <= 1'b1;
                    end else begin
                        comp_init_cnt <= comp_init_cnt + 2'd1;
                    end
                end
            end
        end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (dump_active && dump_force_flush && (write_done || dump_flush_timeout >= 32'd250000000)) begin
                ...
                dump_word_limit <= fre_cnt; // flush完成后按已落盘总条数全量导出
                dump_read_start_addr <= 24'h001000;
                dump_addr <= 24'h001000;
                dump_meta_limit <= fre_cnt;
            end
