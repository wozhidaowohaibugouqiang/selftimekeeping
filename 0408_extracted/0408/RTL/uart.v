...
module uart (
    ...
    input  wire        period_diff_pulse, // 16s 周期脉冲
    ...
    input  wire [15:0] fwe_cnt,          // Flash Write Enable (写入计数)
    // 新增：FIFO读出事件计数器（用于观测读走发生）
    input  wire [15:0] fre_cnt,          // Flash Read Enable (读取计数)
    // 新增：diff_valid脉冲计数（源/写入端）
    input  wire [15:0] dv50_cnt,
    input  wire [15:0] dv25_cnt,         // 差值产生计数
    // 新增：状态变化标志
    ...
    always @(posedge sys_clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            // Heartbeat Logic: every 16s aligned with period_diff_pulse
            if (period_diff_pulse && !dump_mode) begin
                hb_req <= 1'b1;
            end
    ...
    assign dump_word_ready = (tx_state == ST_TX_IDLE) && !reply_req && !hb_req && !cmd_valid && !erase_start_req && !erase_done_req && !read_start_req;
    assign dump_meta_ready = (tx_state == ST_TX_IDLE) && !dump_word_valid && !reply_req && !hb_req && !cmd_valid && !erase_start_req && !erase_done_req && !read_start_req;
    ...
    always @(posedge sys_clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            case (tx_state)
                ST_TX_IDLE: begin
                    uart_tx_en <= 1'b0;
                    // 优先级：按键事件 > 按键元信息 > Dump数据 > Dump结束 > 指令回复 > 心跳包
                    if (erase_start_req) begin
                        tx_state <= ST_TX_PREP;
                        tx_msg_type <= 3'b101; // Erase Start
                    end else if (erase_done_req) begin
                        tx_state <= ST_TX_PREP;
                        tx_msg_type <= 3'b110; // Erase Done
                    end else if (read_start_req) begin
                        tx_state <= ST_TX_PREP;
                        tx_msg_type <= 3'b111; // Read Start
                    end else if (dump_meta_valid && dump_meta_ready) begin
                        tx_state <= ST_TX_PREP;
                ...
                ST_TX_PREP: begin
                    ...
                    if (tx_msg_type == 3'b000) begin
                        ...
                        // DUMP_META:FWE=xxxx,FRE=xxxx,LIMIT=xxxx,EXPT=xxxx\r\n
                        ...
                        tx_buf[38] <= ",";
                        tx_buf[39] <= "E"; tx_buf[40] <= "X"; tx_buf[41] <= "P"; tx_buf[42] <= "T"; tx_buf[43] <= "=";
                        tx_buf[44] <= hex2ascii(dump_meta_exported[15:12]); tx_buf[45] <= hex2ascii(dump_meta_exported[11:8]);
                        tx_buf[46] <= hex2ascii(dump_meta_exported[7:4]);   tx_buf[47] <= hex2ascii(dump_meta_exported[3:0]);
                        tx_buf[48] <= 8'h0D;
                        tx_buf[49] <= 8'h0A;
                        tx_len <= 8'd50;
                        tx_ptr <= 8'd0;
                        tx_state <= ST_TX_SEND;
                    end else if (tx_msg_type == 3'b101) begin
                        // ERASE START
                        tx_buf[0] <= "E"; tx_buf[1] <= "R"; tx_buf[2] <= "A"; tx_buf[3] <= "S"; tx_buf[4] <= "E"; tx_buf[5] <= " ";
                        tx_buf[6] <= "S"; tx_buf[7] <= "T"; tx_buf[8] <= "A"; tx_buf[9] <= "R"; tx_buf[10] <= "T";
                        tx_buf[11] <= 8'h0D; tx_buf[12] <= 8'h0A;
                        tx_len <= 8'd13;
                        tx_ptr <= 8'd0;
                        tx_state <= ST_TX_SEND;
                    end else if (tx_msg_type == 3'b110) begin
                        // ERASE DONE
                        tx_buf[0] <= "E"; tx_buf[1] <= "R"; tx_buf[2] <= "A"; tx_buf[3] <= "S"; tx_buf[4] <= "E"; tx_buf[5] <= " ";
                        tx_buf[6] <= "D"; tx_buf[7] <= "O"; tx_buf[8] <= "N"; tx_buf[9] <= "E";
                        tx_buf[10] <= 8'h0D; tx_buf[11] <= 8'h0A;
                        tx_len <= 8'd12;
                        tx_ptr <= 8'd0;
                        tx_state <= ST_TX_SEND;
                    end else if (tx_msg_type == 3'b111) begin
                        // READ START
                        tx_buf[0] <= "R"; tx_buf[1] <= "E"; tx_buf[2] <= "A"; tx_buf[3] <= "D"; tx_buf[4] <= " ";
                        tx_buf[5] <= "S"; tx_buf[6] <= "T"; tx_buf[7] <= "A"; tx_buf[8] <= "R"; tx_buf[9] <= "T";
                        tx_buf[10] <= 8'h0D; tx_buf[11] <= 8'h0A;
                        tx_len <= 8'd12;
                        tx_ptr <= 8'd0;
                        tx_state <= ST_TX_SEND;
                        ...
                        // 进一步精简心跳字段，降低MSlice占用
                        // 格式: CT,DIFF,FWE,FRE,LW,CA,M
                        ...
                        tx_buf[39] <= "L"; tx_buf[40] <= "W"; tx_buf[41] <= ":";
                        tx_buf[42] <= "0"; tx_buf[43] <= "0";
                        tx_buf[44] <= hex2ascii(last_prog_word[23:20]); tx_buf[45] <= hex2ascii(last_prog_word[19:16]);
                        tx_buf[46] <= hex2ascii(last_prog_word[15:12]); tx_buf[47] <= hex2ascii(last_prog_word[11:8]);
                        tx_buf[48] <= hex2ascii(last_prog_word[7:4]);   tx_buf[49] <= hex2ascii(last_prog_word[3:0]);
                        tx_buf[50] <= ",";

                        tx_buf[51] <= "C"; tx_buf[52] <= "A"; tx_buf[53] <= ":";
                        tx_buf[54] <= comp_fixed_disp[31] ? "-" : "+";
                        tx_buf[55] <= "0";
                        tx_buf[56] <= "0";
                        tx_buf[57] <= hex2ascii(comp_fixed_abs[23:20]);
                        tx_buf[58] <= hex2ascii(comp_fixed_abs[19:16]);
                        tx_buf[59] <= hex2ascii(comp_fixed_abs[15:12]);
                        tx_buf[60] <= hex2ascii(comp_fixed_abs[11:8]);
                        tx_buf[61] <= hex2ascii(comp_fixed_abs[7:4]);
                        tx_buf[62] <= hex2ascii(comp_fixed_abs[3:0]);
                        tx_buf[63] <= ",";

                        tx_buf[64] <= "M"; tx_buf[65] <= ":";
                        tx_buf[66] <= pps_valid ? "L" : "H";
                        tx_buf[67] <= 8'h0D;
                        tx_buf[68] <= 8'h0A;
