...
module flash_controller_enhanced (
    ...
    localparam [15:0] POLL_ERASE_MAX     = 16'd3000;
    ...
    localparam [23:0] DIFF_DATA_BASE     = 24'h001000;  // 训练/回放差值区
    localparam [23:0] DIFF_DATA_END      = 24'h00FFFF;
    localparam [23:0] PARAM_DATA_BASE    = 24'h010000;  // 老化参数区
    localparam [7:0]  TABLE_MAGIC        = 8'h55;
    localparam [5:0]  TABLE_ENTRY_SIZE   = 6'd64;
    ...
    wire        need_erase = 1'b0; // (erased_sector_base != sector_base);
    ...
    always @(posedge sys_clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            erase_pending       <= 1'b0;
            erase_sector_cur    <= DIFF_DATA_BASE;
            ...
            case (state)
                ...
                ST_SEND_ERASE: begin
                    if (txn_done) begin
                        if (!erase_wren_checked) begin
                            ...
                            txn_start <= 1'b1;
                            erased_sector_base <= sector_base;
                            poll_attempts      <= 16'd0;
                            poll_wait          <= 24'd0;
                            erase_poll_issued  <= 1'b0;
                            state              <= ST_POLL_ERASE;
                ...
                ST_CHECK_FIFO: begin
                    if (cur_addr >= DIFF_DATA_END) begin
                        state <= ST_DONE;
                    // --- 修改：平时不写，只有满了或触发flush时才批量写 ---
                    // 仅当 fifo_full_flag (afull 或 flush_req) 拉高时，才开始处理FIFO数据
                    // 旧逻辑注释保留: else if (!fifo_empty_flag) begin
                    end else if (fifo_full_flag && !fifo_empty_flag) begin
                        state <= ST_FIFO_RE;
                    end else begin
                        state <= ST_DONE;
                    end
                    ...
                    if (txn_done) begin
                        if (!prog_wren_checked) begin
                            ...
                            if (param_write_pending) begin
                                ...
                                // 根据prog_byte_idx选择要写的字节
                                case (prog_byte_idx)
                                    2'd0: txn_b4 <= prog_word[31:24];  // 字节0
                                    2'd1: txn_b4 <= prog_word[23:16];  // 字节1
                                    2'd2: txn_b4 <= prog_word[15:8];   // 字节2
                                    default: txn_b4 <= prog_word[7:0]; // 字节3
                                endcase
                            end
                ...
                ST_ERASE_ALL_NEXT: begin
                    ...
                    if (erase_sector_cur + 24'h1000 <= DIFF_DATA_END) begin
