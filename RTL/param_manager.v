...
module param_manager (
    ...
    // 秒计数器（生成时间戳）
    // ========================================================================
    localparam [25:0] SEC_COUNT_MAX = 26'd49999999; // 50MHz时钟，1秒
    reg [25:0] sec_counter_reg;

    always @(posedge sys_clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            sec_counter_reg <= 26'd0;
            sec_tick <= 1'b0;
            ...
            if (sec_counter_reg >= SEC_COUNT_MAX) begin
                ...
                sec_counter_reg <= sec_counter_reg + 26'd1;
                sec_tick <= 1'b0;
            end
        end
    end
        ...
        if (!sys_rst_n) begin
            ...
            case (state)
                ...
                ST_BUILD_META: begin
                    ...
                    if (meta_index < 5'd15) begin
                        meta_index <= meta_index + 5'd1;
                    end else begin
                ...
                ST_WRITE_PARAM: begin
                    // 输出参数数据给Flash控制器
                    if (meta_index < 5'd16) begin
                        param_data <= meta_buffer[meta_index[3:0]];
                        param_type <= TYPE_SINGLE;
