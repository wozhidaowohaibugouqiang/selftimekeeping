...
module spi_driver (
    ...
    output reg [7:0] txn_rx5,
    output reg [7:0] txn_rx6,
    output reg [7:0] txn_rx7,
    ...
    always @(posedge sys_clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            len_lat    <= 3'd0;
            cs_delay_cnt <= 4'd0;
            {txn_rx0, txn_rx1, txn_rx2, txn_rx3, txn_rx4, txn_rx5, txn_rx6, txn_rx7} <= 64'd0;
        end else begin
            txn_done <= 1'b0;
            ...
            case (state)
                ST_IDLE: begin
                    ...
                    if (txn_start && !txn_busy) begin
                        txn_busy <= 1'b1;
                        tx_shift <= {txn_b0, txn_b1, txn_b2, txn_b3, txn_b4, txn_b5, txn_b6, txn_b7};
                        len_lat  <= txn_len;
                        case (txn_len)
                            3'd0: total_bits <= 7'd64;
                            3'd1: total_bits <= 7'd8;
                            3'd2: total_bits <= 7'd16;
                            3'd3: total_bits <= 7'd24;
                            3'd4: total_bits <= 7'd32;
                            default: total_bits <= 7'd40;
                        endcase
                        bit_cnt <= 7'd0;
                        state   <= ST_CS_LOW;
                    end
                ...
                ST_CLK_LO: begin
                    ...
                    if (bit_cnt + 7'd1 >= total_bits) begin
                        state <= ST_CS_HIGH;
                    end else begin
                ...
                ST_CS_HIGH: begin
                    ...
                    case (len_lat)
                        3'd0: begin
                            txn_rx0 <= rx_shift[63:56];
                            txn_rx1 <= rx_shift[55:48];
                            txn_rx2 <= rx_shift[47:40];
                            txn_rx3 <= rx_shift[39:32];
                            txn_rx4 <= rx_shift[31:24];
                            txn_rx5 <= rx_shift[23:16];
                            txn_rx6 <= rx_shift[15:8];
                            txn_rx7 <= rx_shift[7:0];
                        end
                        3'd1: begin
